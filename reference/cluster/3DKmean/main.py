...
def process(xyz_data):
    ...
    for center in centers:
        ...
        for point in data:
            if (is_point_in_box(point, x_min, x_max, y_min, y_max, z_min, z_max) and point not in points):
                points.append(point)
                isInBoxNum += 1
    ...
    print("采收率: {} / {} = {}".format(isInBoxNum, len(data), rate))
    return rate
...
if "__main__" == __name__:
    # 配置原先设定
    K = 3   # 设置采收次数
    BOUNDING_BOXES_H = 0.70  # 设定作业范围 高
    BOUNDING_BOXES_W = 0.12  # 设定作业范围 宽
    BOUNDING_BOXES_L = 0.35  # 设定作业范围 长
